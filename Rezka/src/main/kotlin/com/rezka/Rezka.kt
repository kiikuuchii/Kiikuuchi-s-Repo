...
class Rezka : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        doc.select("table.b-post__info tr").forEach { tr ->
            ...
            when {
                ...
                key.contains("Режиссер", true) -> {
                    valueCell?.select("a")?.forEach { a ->
                        val n = a.text().trim()
                        if (n.isNotEmpty()) actors.add(ActorData(Actor(n), role = null)) // role = null вместо Director
                    }
                }
                key.contains("В ролях", true) -> {
                    valueCell?.select("a")?.forEach { a ->
                        val n = a.text().trim()
                        if (n.isNotEmpty()) actors.add(ActorData(Actor(n), role = null)) // role = null вместо Actor
                    }
                }
            }
        }
        ...
        val isSeries =
            url.contains("/series", true) || url.contains("/serial", true) ||
            doc.select("#simple-episodes-list, .b-simple_episode__list, .b-simple_episodes__list").isNotEmpty()
        ...
        return when (type) {
            ...
            else -> null // обязательная ветка для exhaustiveness
        }
    }
}